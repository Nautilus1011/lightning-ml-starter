...
.
├── src/
│   ├── configs/                 # Hydra 設定ファイル (YAML)
│   │   └── config.yaml          # 学習の全パラメータをここで管理
│   ├── detection_toolkit/       # メインパッケージ
│   │   ├── datamodules/
│   │   │   └── voc_datamodule.py  # Pascal VOC のダウンロード・前処理・DataLoader
│   │   └── models/
│   │       └── detector.py        # LightningModule (Faster R-CNN ラッパー)
│   ├── train.py                 # 学習エントリーポイント
│   └── inference.py             # 推論エントリーポイント
├── data/                        # VOC データセット保存先 (初回自動DL)
├── outputs/                     # チェックポイント・ログ出力先
├── scripts/
│   └── smoke_test.py            # パイプライン動作確認スクリプト
├── Dockerfile
├── docker-compose.yml
└── requirements-dev.txt
```
...
> GPU を使う場合は事前に [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html) をインストールしてください。
...
> GPU がない環境でも `docker compose up -d` はそのまま動作します（CPU モードで実行されます）。

---

## 2. Weights & Biases (wandb) アカウントの設定

wandb は学習曲線・バウンディングボックス付き画像・ハイパーパラメータを自動でログする実験管理ツールです。

### アカウント作成

1. [https://wandb.ai/site](https://wandb.ai/site) にアクセスして "Sign up"
2. GitHub / Google アカウント連携 またはメールで登録
3. ログイン後、右上アバター → **User Settings** → **API keys** → "New key" でキーをコピー

### Docker 内での認証（2通りの方法）

**方法 A — コンテナ起動前に `.env` ファイルで渡す（推奨）**

プロジェクトルートに `.env` ファイルを作成し、APIキーを書いておくと `docker compose up` 時に自動で注入されます。

```bash
# .env ファイルをプロジェクトルートに作成（git には含めないこと）
echo "WANDB_API_KEY=ここにAPIキーを貼り付け" > .env
```
...
# コンテナ内で実行
wandb login
# → プロンプトが出たら API キーをペーストして Enter
# → "Successfully logged in" と表示されれば完了
```
...
コンテナ内の `/app/src` から実行します。

```bash
cd /app/src

# 基本実行（config.yaml の設定がそのまま使われる）
PYTHONPATH=/app/src python train.py
```

### Hydra によるパラメータ上書き

コマンドラインからパラメータをその場で変更できます。`config.yaml` を書き換える必要はありません。

```bash
# バッチサイズを変更、エポック数を増やす
PYTHONPATH=/app/src python train.py data.batch_size=8 trainer.max_epochs=20

# 学習率を変更
PYTHONPATH=/app/src python train.py model.lr=0.001
```

### wandb なしでオフライン実行（開発・デバッグ時）

```bash
WANDB_MODE=offline PYTHONPATH=/app/src python train.py
```

### WandB Sweep（自動ハイパーパラメータ探索）の例

以下の内容を `sweep.yaml` として保存し、Sweep を実行します。
...
```bash
cd /app/src
wandb sweep sweep.yaml          # → sweep ID が発行される
wandb agent <your-sweep-id>     # 自動探索開始
```

### wandb ダッシュボードで確認できる内容

学習中は [https://wandb.ai](https://wandb.ai) のダッシュボードに以下がリアルタイムで記録されます。

- `train_loss` / `val_loss` のグラフ
- 各損失成分（`train_loss_classifier`, `train_loss_box_reg` など）
- 学習率の変化（`LearningRateMonitor`）
- バリデーション時の**バウンディングボックス付き予測画像**（スコア > 0.5 の予測のみ表示）
- `EarlyStopping` が発動した場合のエポック数

> **補足**: VOC 2012 データセット（約 2 GB）は初回実行時に `data/` に自動ダウンロードされます。

---

## 4. 推論の実行

学習完了後、保存されたチェックポイント (`.ckpt`) を使って任意の画像を推論できます。
...
チェックポイントのパスは `src/configs/config.yaml` の `callbacks.model_checkpoint.filename` で指定した名前に合わせてください（デフォルトは `best-checkpoint.ckpt`）。

---

## 5. パイプライン動作確認（スモークテスト）

VOC データをダウンロードせず、合成データでパイプライン全体（学習 → チェックポイント保存 → 推論）を素早く確認したい場合は以下を使います。
...
## 設定ファイル (`src/configs/config.yaml`)

```yaml
logger:
  project_name: "voc-object-detection"   # wandb プロジェクト名
...
model:
  num_classes: 21    # VOC 20クラス + 背景 1クラス
  lr: 0.005          # Sweep で自動探索する主なターゲット
  momentum: 0.9
  weight_decay: 0.0005

trainer:
  max_epochs: 10
  accelerator: "auto"    # GPU があれば自動で使用
  devices: "auto"
  precision: "32"        # "16-mixed" にするとGPU使用時に高速化
